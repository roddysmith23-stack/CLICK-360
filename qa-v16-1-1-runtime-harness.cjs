...
const assetVersion = 'mvp-launch-v16-1-1-r1';
...
assert.equal(firefoxQa.context.CLICK360_RUNTIME_GUARD.APP_VERSION, '16.1.1');
...
errorListener({
  ...
  lineno: 1,
  colno: 1,
  error: { stack: 'content.js?inviteToken=secret:1:1' }
});
...
assert.equal(reports[0].appVersion, '16.1.1');
...
assert(app.includes('V16.1.1 · Control total de tu negocio'));
assert(firebase.includes('<small>V16.1.1</small>'));
...
console.log('PASS V16.1.1 runtime: safe Firefox detection, sanitized reports, tenant namespaces and friendly UX');
console.log('PASS V16.1.1 cache: all scripts versioned, old cache removed and no unresolved first-party build constant');
console.log('PASS V16.1.1 Shary contract: ONLINE_ONLY_SAFE uses transactional V10 bootstrap without STATE_DOC.set');
