...
const url = `http://127.0.0.1:${port}/?v=commercial-1-0-5-r37-2-4-cloud-confirmed-r2`;
...
async function run(name, browserType, options = {}) {
  ...
  try {
    ...
    if (release?.assetVersion !== 'commercial-1-0-5-r37-2-4-cloud-confirmed-r2') throw new Error(`${name} wrong asset version: ${JSON.stringify(release)}`);
    if (!release?.buildSha || release.buildSha === '__CLICK360_BUILD_SHA__') throw new Error(`${name} build SHA was not injected`);
    const manifest = await page.evaluate(async () => {
      const response = await fetch('manifest.webmanifest?v=commercial-1-0-5-r37-2-4-cloud-confirmed-r2');
      return { ok:response.ok, data:await response.json() };
    });
    if (!manifest.ok || !String(manifest.data.start_url || '').includes('commercial-1-0-5-r37-2-4-cloud-confirmed-r2')) {
    ...
    if (name === 'chromium') {
      ...
      if (!caches.includes('click360-commercial-1-0-5-r37-2-4-cloud-confirmed-r2')) {
