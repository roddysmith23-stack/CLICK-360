...
const url = `http://127.0.0.1:${port}/?v=commercial-1-0-5-r15`;
...
async function run(name, browserType, options = {}) {
  ...
  try {
    ...
    if (release?.assetVersion !== 'commercial-1-0-5-r15') throw new Error(`${name} wrong asset version: ${JSON.stringify(release)}`);
    if (!release?.buildSha || release.buildSha === '__CLICK360_BUILD_SHA__') throw new Error(`${name} build SHA was not injected`);
    const manifest = await page.evaluate(async () => {
      const response = await fetch('manifest.webmanifest?v=commercial-1-0-5-r15');
      return { ok:response.ok, data:await response.json() };
    });
    if (!manifest.ok || !String(manifest.data.start_url || '').includes('commercial-1-0-5-r15')) {
    ...
    if (name === 'chromium') {
      ...
      if (!caches.includes('click360-commercial-1-0-5-r15')) {
