...
```bash
node tools/admin/scripts/migrate-legacy-v9-to-v10.mjs --project=click-360 --dry-run --businessId=BUSINESS_ID
```

## Apply one audited tenant

```bash
node tools/admin/scripts/migrate-legacy-v9-to-v10.mjs --project=click-360 --apply --businessId=BUSINESS_ID
```

## Apply an audit-reviewed allowlist

```bash
node tools/admin/scripts/migrate-legacy-v9-to-v10.mjs --project=click-360 --apply --allowlist=path/to/approved-tenants.json
```
