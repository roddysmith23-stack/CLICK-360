...
Restaurante Avanzado is a feature-gated extension of Mesas Lite. The existing `tables` and `tableOrders` paths remain untouched and compatible. The advanced candidate stores its new synthetic state in `restaurantOrders`, `restaurantPayments`, `restaurantPrintHistory`, and `restaurantEvents`, each scoped by `businessId`.

The flag is named `restaurantAdvancedEnabled`. It starts disabled, requires an explicitly allowlisted business, a restaurant-type business, and `business.settings.modules.restaurant === true`. No production flag, Rule, Auth, account access, claim, or customer tenant was changed.
...
This candidate does not add online reservations, customer QR ordering, delivery marketplace integrations, recipes, fiscal invoicing, or external accounting. It does not claim physical printer certification.
