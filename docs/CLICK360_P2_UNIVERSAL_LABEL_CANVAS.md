...
Status: `READY_FOR_SHARY_PHYSICAL_SMOKE`

This work is intentionally stacked on
`architecture/p2-web-admin-dependency-boundary` (PR #30). The corresponding
canvas PR remains a Draft; neither branch changes a Firebase environment or
publishes a client release.
...
browser E2E. Administrative security remains independently visible in PR #30;
it is not part of this canvas diff and its jobs must be skipped rather than
reported as passed by this PR.
