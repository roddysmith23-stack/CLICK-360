...
assert.match(app, /const result = action \? \{[\s\S]*quantity:[\s\S]*startSlot:[\s\S]*\} : \{ confirmed:false \};[\s\S]*closeModal\(\)/, 'quick print reads inputs before closing the modal');
assert.match(app, /id="quickLabelPdf"/, 'PDF remains distinct from device printing');
assert.match(app, /id="quickLabelPrint"/, 'device printing has its own action');
