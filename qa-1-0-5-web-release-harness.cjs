...
assert.match(app, /APP_ASSET_VERSION = 'commercial-1-0-5-r6'/);
assert.match(index, /universal-label-canvas\.js\?v=commercial-1-0-5-r6/);
assert.match(index, /universal-label-editor\.js\?v=commercial-1-0-5-r6/);
assert.match(worker, /click360-commercial-1-0-5-r6/);
...
assert.match(app, /requestAnimationFrame\(\(\) => targetPanel\.scrollIntoView\(\{ block:'start', inline:'nearest' \}\)\)/);
...
assert.match(styles, /\.labelPreviewSticky canvas\{max-height:clamp\(76px,14dvh,112px\)\}/);
