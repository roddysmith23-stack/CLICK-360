...
assert.match(app, /APP_ASSET_VERSION = 'commercial-1-0-5-r16'/);
assert.match(index, /By AIIA INTELLIGENCE TECHNOLOGIES/, 'startup splash shows the required AIIA footer');
assert.match(index, /assets\/logo\.png\?v=commercial-1-0-5-r16/, 'startup splash uses the HD logo instead of the low-resolution favicon');
assert.match(index, /rel="preload" as="image" href="assets\/logo\.png\?v=commercial-1-0-5-r16"/, 'startup HD logo is preloaded before scripts');
...
assert.match(index, /universal-label-canvas\.js\?v=commercial-1-0-5-r16/);
assert.match(index, /universal-label-editor\.js\?v=commercial-1-0-5-r16/);
assert.match(index, /p2-restaurant-domain\.js\?v=commercial-1-0-5-r16/);
assert.match(index, /p2-logistics-domain\.js\?v=commercial-1-0-5-r16/);
assert.match(worker, /click360-commercial-1-0-5-r16/);
