...
const assetVersion = 'mvp-launch-v16-2-p2-web-safe-r1';
...
assert.equal(firefoxQa.context.CLICK360_RUNTIME_GUARD.APP_VERSION, '1.0.5-p2-web-safe');
...
assert.equal(reports[0].appVersion, '1.0.5-p2-web-safe');
...
assert(firefoxQa.toast.children.some((node) => node.href?.includes('Version%3A%201.0.5-p2-web-safe%20abc123def456')), 'support link includes release version and SHA');
...
assert(app.includes("const APP_RELEASE_VERSION = '1.0.5-p2-web-safe'") && app.includes('APP_VISIBLE_VERSION') && app.includes('class="brandSlogan">Control total de tu negocio</small>'));
